...
class _EsignScreenState extends State<EsignScreen> {
                  ...
                  child: FlatButton(
                    ...
                    onPressed: docID==""?null:() async {
                      if (Platform. isAndroid) {
                        bool es = await startEsigning(docID,esignToken);
  ...
  Future<void> GeneratePDFAndGetEsignDocID() async{
    ...
    Map valueMap = await ApiRepository().Digio_eSign_Document_Upload();
    docID = valueMap["res_Output"][0]["result_Description"];
    esignToken = valueMap["res_Output"][0]["result_Extra_Key"];
  ...
  Future<bool> startEsigning(String docID,String esignToken) async {
    bool result = false;
    ...
    try {
      result= await platform.invokeMethod('esignThisDocument',{"docID":docID,"phoneNumber":phoneNumber,"esignToken":esignToken});
